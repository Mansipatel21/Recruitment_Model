...
if st.session_state.logged_in and st.session_state.user_role:
    with st.sidebar:
        if st.session_state.user_role == "Job-Seeker":
            selected = option_menu("Home",
                                   ["Dashboard", "Resume Upload","User Profile","Job Matching", "Recruitment Analytics", "About Us", "Log out"],
                                   icons=["cast", "folder","person","search","bar-chart", "people", "box-arrow-right"],
            ...
            selected = option_menu("Home", ["Dashboard", "Job Upload", "Job Openings","Candidate Matching","Visualization", "About-Us", "Log out"],
                                   icons=["cast", "briefcase", "book","search","bar-chart", "people", "box-arrow-right"],
                                   menu_icon="house", orientation="vertical",styles={"nav-link-selected": {
                                   "background-color": "#9D8A7C",
                                   "color": "white",
                                   "border-radius": "8px"}})
    ...
    if st.session_state.user_role == "Job-Seeker" and selected == "Dashboard":
        ...
        if st.button("Submit"):
            if not name or not email or not phone or not skills or not industry:
                st.error("All fields are required!")
                ...
                with open("form_data.csv", "a", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow([name, email, phone, age, education, skills, experience, industry, projects])
        ...
        if st.button("Find Matching Candidates"):
            ...
            if matching_jobs.empty:
                ...
                for idx, score in sorted_candidates[:20]:
                    ...
                    st.markdown(f"**LinkedIn link:** {candidate['linkedin']}")
                    st.markdown(f"**GitHub link:** {candidate['github']}")
